...
import { sfxCatch, sfxSelect, startMusicMonto, stopMusic } from '../utils/sound'
...
export default function MiniGameMonto({ onWin }) {
  ...
  useEffect(() => {
    ...
    const kd = e => { keys[e.key] = true; if (e.key === ' ' || e.key === 'Space') e.preventDefault() }
    ...
    function update() {
      ...
      for (let i = falling.length - 1; i >= 0; i--) {
        ...
        f.y += 2.5
        if (f.y > H) { falling.splice(i, 1); continue }
        if (f.y + f.h >= basket.y && f.y <= basket.y + basket.h &&
          ...
          setSum(currentSum)
          floats.push({ x: basket.x + basket.w / 2, y: basket.y - 5, value: f.value, life: 40, maxLife: 40 })
          sfxCatch()
        }
      }
    ...
    function draw() {
      ...
      for (const fl of floats) {
        ...
        ctx.fillText('+$' + fl.value, fl.x, fl.y)
      }
      ctx.globalAlpha = 1
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: 'center', marginTop: '12px' }}>
        <button
          onClick={() => { sfxSelect(); onWinRef.current?.(sumRef.current) }}
          style={{
            padding: '10px 32px',
